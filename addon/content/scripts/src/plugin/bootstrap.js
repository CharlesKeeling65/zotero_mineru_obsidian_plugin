...
class PluginManager {
    ...
    async initialize() {
        ...
        try {
            ...
            defaultLogger.error("插件初始化失败", { error: error.message });
            throw error;
        }
    }
    /**
    ...
    initializeReaderToolbar() {
        ...
        initializeReaderToolbar(async (pdfPath, zoteroItemKey, title) => {
            try {
                ...
                defaultLogger.error("PDF 解析失败", { error: error.message, pdfPath });
                this.showNotification("解析失败", `解析 ${title} 时出错: ${error.message}`);
            }
        });
    }
    /**
...
export function bootstrapPlugin() {
    return {
        ...
        initializeReaderToolbar: (provider, translationProvider, ragIntegration) => {
