...
export class RagClient {
     ...
     */
    async indexDocument(itemKey, documentJsonPath, force = false) {
        try {
    ...
    async getDocumentBlocks(itemKey, limit = 100, offset = 0) {
        try {
            const response = await this.fetchWithRetry(`/document/${itemKey}/blocks?limit=${limit}&offset=${offset}`);
            return response;
        }
        catch (error) {
    ...
    async fetchWithRetry(endpoint, options = {}) {
        ...
        for (let attempt = 0; attempt < this.config.retries; attempt++) {
            try {
                ...
                lastError = error;
                console.warn(`RAG service request failed (attempt ${attempt + 1}/${this.config.retries}):`, error);
 ...
 */
export function createRagClientFromSettings(settings) {
