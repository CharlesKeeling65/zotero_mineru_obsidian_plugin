 ...
 */
export function convertBlockToChunks(
 ...
 */
function convertBlockToParagraphChunks(
  input: CreateChunkInput,
  config: ChunkConfig
): Chunk[] {
  const { block, context, itemKey, documentId, embeddingModel } = input;
 ...
 */
function convertBlockToSectionChunks(
  input: CreateChunkInput,
  config: ChunkConfig
): Chunk[] {
 ...
 */
function convertBlockToMergedChunks(
  input: CreateChunkInput,
  config: ChunkConfig
): Chunk[] {
 ...
 */
function convertBlockToSplitChunks(
  input: CreateChunkInput,
  config: ChunkConfig
): Chunk[] {
  const { block, context, itemKey, documentId, embeddingModel } = input;
 ...
 */
export function convertBlocksToChunks(
  blocks: Block[],
  itemKey: string,
  documentId: string,
  config: ChunkConfig = DEFAULT_CHUNK_CONFIG,
 ...
 */
function mergeShortChunks(chunks: Chunk[], config: ChunkConfig): Chunk[] {
  if (chunks.length === 0) {
    return [];
  }
