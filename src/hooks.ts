...
let provider: MineruProvider | null = null;
...
export const hooks = {
  ...
  registerReaderEventListener() {
    Zotero.Reader.registerEventListener("DOMContentLoaded", (event: any) => {
   ...
   */
  injectReaderToolbarButton(reader: any, doc: Document) {
   ...
   */
  async handleParseButtonClick(reader: any) {
    if (!provider) {
    ...
    try {
      ...
      defaultLogger.error("PDF parse failed", { error: (error as Error).message });
      Zotero.Notifier.notify({
        type: "message",
        title: "Parse Failed",
        message: `Error parsing ${title}: ${(error as Error).message}`,
        timeout: 8000,
      });
    }
  },
  ...
  registerItemPaneSection() {
    try {
      Zotero.ItemPaneManager.registerSection({
        ...
        body: {
          onRender: (doc: Document, container: HTMLElement) => {
      ...
      defaultLogger.warn("Could not register item pane section", { error: (error as Error).message });
    }
  },
   ...
   */
  renderItemPaneSection(doc: Document, container: HTMLElement) {
    container.innerHTML = `
